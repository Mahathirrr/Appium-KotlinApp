...
public class LoginTest extends BaseTest {
    ...
    public void testLoginPageElements() {
        createTest("Login Page Elements Test",
                "Verify all login page elements are displayed correctly");

        createTest("Test Method", "Test Description");
        logInfo("Checking if login page is loaded");
        if (loginPage.isPageLoaded()) {
            logPass("Login page is loaded successfully");
        } else {
            logInfo("Login page not detected - taking screenshot for analysis");
            takeScreenshot("login_page_not_detected");
            return;
        }

        takeScreenshot("login_page_loaded");

        logInfo("Verifying login page title");
        String pageTitle = loginPage.getPageTitle();
        logInfo("Page title: " + pageTitle);

        logInfo("Checking email field");
        if (loginPage.isEmailFieldDisplayed()) {
            logPass("Email field is displayed");
        } else {
            logInfo("Email field not found");
        }

        logInfo("Checking password field");
        if (loginPage.isPasswordFieldDisplayed()) {
            logPass("Password field is displayed");
        } else {
            logInfo("Password field not found");
        }

        logInfo("Checking login button");
        if (loginPage.isLoginButtonEnabled()) {
            logPass("Login button is displayed and enabled");
        } else {
            logInfo("Login button not found or not enabled");
        }

        logInfo("Checking Google Sign In button");
        if (loginPage.isGoogleSignInDisplayed()) {
            logPass("Google Sign In button is displayed");
        } else {
            logInfo("Google Sign In button not found");
        }

        logInfo("Checking register link");
        if (loginPage.isRegisterLinkDisplayed()) {
            logPass("Register link is displayed");
        } else {
            logInfo("Register link not found");
        }

        logInfo("Checking back button");
        if (loginPage.isBackButtonDisplayed()) {
            logPass("Back button is displayed");
        } else {
            logInfo("Back button not found");
        }

        logInfo("Checking password toggle");
        if (loginPage.isPasswordToggleDisplayed()) {
            logPass("Password toggle is displayed");
        } else {
            logInfo("Password toggle not found");
        }
    }
